...
public class ArquivoTarefas extends Arquivo<Tarefa> {
  ...
  private HashMap<Integer, List<Integer>> tarefaParaRotulos; // TP3
  ...
  public ArquivoTarefas() throws Exception {
    ...
    tarefaParaRotulos = new HashMap<>(); // TP3
  ...
  // Método que remove os acentos e tira as letras maiúsculas e retorna um
  // arraylist com as palavras chave
  ...
  public int create(Tarefa obj) throws Exception {
    ...
    if (tarefaParaRotulos.containsKey(id)) { // TP3
      List<Integer> rotulos = tarefaParaRotulos.get(id);
      for (int rotuloId : rotulos) {
        incluirRotulo(id, rotuloId);
      }
    }
  ...
  // Métodos de rótulo
  public void incluirRotulo(int idTarefa, int idRotulo) {
    tarefaParaRotulos.computeIfAbsent(idTarefa, k -> new ArrayList<>()).add(idRotulo);
  }

  public List<Integer> buscarTarefasPorRotulo(int idRotulo) {
    return tarefaParaRotulos.getOrDefault(idRotulo, new ArrayList<>());
  }

  public void removerRotulo(int idTarefa, int idRotulo) {
    List<Integer> rotulos = tarefaParaRotulos.get(idTarefa);
    if (rotulos != null) {
        rotulos.remove((Integer) idRotulo);
        if (rotulos.isEmpty()) {
            tarefaParaRotulos.remove(idTarefa);
        }
    }
}
